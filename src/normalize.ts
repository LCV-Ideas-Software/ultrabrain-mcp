...
export function normalizeThoughtInput(args: unknown): ThoughtInput {
  ...
  const input: ThoughtInput = {
    ...
    mode: parseEnum(
      reasoningModeSchema,
      pick(raw, "mode", "reasoningMode", "ultrabrainMode"),
      "hybrid",
    ),
    ...
    budget_mode: parseEnum(
      budgetModeSchema,
      pick(raw, "budget_mode", "budgetMode"),
      "balanced",
    ) as BudgetMode,
...
export function readSessionId(args: unknown): string {
  ...
  return (
    asString(pick(args as Record<string, unknown>, "session_id", "sessionId")) ?? DEFAULT_SESSION_ID
  );
}
...
function validateThoughtRules(input: ThoughtInput): void {
  ...
  if (hasBranchId !== hasBranchFrom) {
    throw new Error(
      "Branching requires both branch_id/branchId and branch_from_thought/branchFromThought.",
    );
  }
  ...
  if (
    input.depth_level !== undefined &&
    (!Number.isInteger(input.depth_level) || input.depth_level < 1)
  ) {
    throw new Error("depth_level/depthLevel must be a positive integer.");
  }

  if (
    input.max_depth !== undefined &&
    (!Number.isInteger(input.max_depth) || input.max_depth < 1)
  ) {
    throw new Error("max_depth/maxDepth must be a positive integer.");
  }

  if (
    input.depth_level !== undefined &&
    input.max_depth !== undefined &&
    input.depth_level > input.max_depth
  ) {
    throw new Error("depth_level/depthLevel cannot exceed max_depth/maxDepth.");
  }
