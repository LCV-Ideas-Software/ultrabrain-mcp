...
const tools = [
  {
    ...
    description: 'Create a branded LCV Ultrabrain reasoning session and optionally seed the first thought.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
        ...
        template: { type: 'string', description: 'Optional template id from ultrabrain_templates.' },
        tags: { type: 'array', items: { type: 'string' } },
        initial_thought: { type: 'string', description: 'Optional first thought.' },
        total_thoughts: { type: 'number', description: 'Estimated total thoughts for the seeded chain.' },
        response_format: responseFormatProperty,
      },
      required: ['problem'],
    },
  ...
  },
  {
    ...
    description: 'Update an existing thought with stronger evidence, confidence, risks, actions, or quality metrics.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: thoughtProperties,
      required: ['thought', 'thought_number', 'total_thoughts', 'next_thought_needed', 'branch_from_thought', 'branch_id'],
    },
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  },
  {
    ...
    description: 'Merge one or more branch insights into a synthesis, best-evidence path, or decision note.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
        ...
        create_thought: { type: 'boolean', description: 'Whether to append the synthesis as a new thought.' },
        response_format: responseFormatProperty,
      },
      required: ['branch_ids'],
    },
  ...
  },
  {
    ...
    description: 'Check a reasoning session for evidence, alternatives, verification, bias, and closure gaps.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
  ...
  },
  {
    ...
    description: 'Analyze quality, confidence, bias counts, label counts, unresolved questions, and recommendations.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
  ...
  },
  {
    ...
    description: 'Return aggregate session, thought, branch, quality, confidence, bias, mode, and step metrics.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: sessionIdProperty,
...
const prompts = [
  {
    ...
    description: 'Break a technical problem into evidence, assumptions, hypotheses, verification, and risks.',
    arguments: [
  ...
  },
  {
    ...
    description: 'Review an implementation or decision with counterexamples, residual risk, and verification checks.',
    arguments: [
      { name: 'subject', description: 'Implementation, design, or decision to review.', required: true },
...
const resources = [
  {
    ...
    description: 'Current in-memory Ultrabrain sessions.',
...
const resourceTemplates = [
  {
    ...
    description: 'A specific in-memory Ultrabrain session.',
    mimeType: 'application/json',
  },
  {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === 'ultrabrain_merge') {
      ...
      return objectResponse(engine.merge(input), input.response_format);
    }
    ...
    if (name === 'ultrabrain_reset') {
      return jsonResponse(engine.reset(normalizeSessionId(args), readBoolean(args, 'all_sessions') ?? false));
    }
...
function errorResponse(error: unknown) {
  ...
  const payload: ToolErrorPayload = {
    status: 'failed',
    error: message,
    guidance: 'Use the LCV Ultrabrain tool schema with ultrabrain_* tool names and en-US field descriptions.',
    example: {
      thought: 'Map the problem, list evidence, compare alternatives, and choose the next verification step.',
      thought_number: 1,
      total_thoughts: 4,
...
function readResponseFormat(args: unknown): ResponseFormat {
  const format = readString(args, 'response_format') ?? readString(args, 'responseFormat') ?? 'json';
